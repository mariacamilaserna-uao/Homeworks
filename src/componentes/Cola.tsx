...
export function Cola({ personas }: ColaProps) {
  const personasOrdenadas = [...personas].sort((a, b) => 
    a.fechaLlegada.getTime() - b.fechaLlegada.getTime()
  );
          ...
          {personasOrdenadas.map((persona, indice) => (
            <li key={persona.id} className="cola-item">
              ...
              <div className="info">
                ...
                <p className="monto">${persona.monto.toLocaleString('es-CO')}</p>
              </div>
              <div className="tiempos">
                <span className="hora">{formatearFecha(persona.fechaLlegada)}</span>
                <span className="espera">Espera: {calcularTiempoEspera(persona.fechaLlegada)}</span>
              </div>
            </li>
          ))}
        </ul>
